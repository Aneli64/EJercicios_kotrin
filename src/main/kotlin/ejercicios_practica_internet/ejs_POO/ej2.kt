...
class Password(var longitud: Int, var contraseña: String)
{
    fun esFuerte(): Boolean
    {
        ...
        var contMinus = 0
        var contNum = 0
        for (item in contraseña)
        {
            when
            {
                item.toString() in  "A".."Z" -> contMayus += 1
                item.toString() in  "a".."z" -> contMinus += 1
                item.toString() in  "0".."9" -> contNum += 1
            }
...
fun main()
{
    val passw = Password(11, "QERty879705")
    if (passw.esFuerte()) println("Contraseña fuerte") else println("Contraseña débil")
}
