...
class Almacen(var estanterias: MutableMap<Int, MutableList<Bebida>>) {
    ...
    fun AgregarProductoMenorEst(producto: Bebida) { //al estante menor
        ...
        for ((c, v) in estanterias) if (v.size == tamanioEst.min()) v.add(producto)
    }

    fun AgregarProductoMarca(producto: Bebida) { //al estante con mas productos de su marca
        var contEstMarca = 0
        var tamanioEstMarca = mutableListOf<Int>()
        for ((c, v) in estanterias) {
            for (item in v) if (item.marca == producto.marca) contEstMarca++
            tamanioEstMarca.add(contEstMarca)
            contEstMarca = 0
        }
        estanterias[tamanioEstMarca.max()]?.add(producto)
    }
...
fun main() {
    ...
    val agua2 = AguaMineral(5, 2.2, 4.0, "Fontbella", "Manantial")
    ...
    //almacen.AgregarProductoMenorEst(aquarius)

    val cocaCola2 = BebidaAzucarada(9, 1.5, 2.0, "Coca-Cola", 25.0, false)
    almacen.AgregarProductoMarca(cocaCola2)
